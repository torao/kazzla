...
import org.msgpack.packer.{Packer, BufferPacker}
import org.msgpack.`type`._
import com.kazzla.domain.irpc.Close
import com.kazzla.domain.irpc.Control
import com.kazzla.domain.irpc.Open
import com.kazzla.domain.irpc.Block
import java.lang
...
object MsgPackCodec extends Codec {
	...
	def pack(unit:Transferable):Array[Byte] = {
		...
		unit match {
			...
			case close:Close =>
				...
				packer.write(close.errorMessage)
	...
	def unpack(buffer:RawBuffer):Option[Transferable] = {
		...
		val packer = new MessagePack().createBufferUnpacker(buffer.raw, buffer.offset, buffer.length)
		val version = packer.readByte()
		packer.readByte() match {
			case OPEN =>
	...
	private[this] def toValue(v:Any):Value = v match {
		...
		case flag:Boolean => ValueFactory.createIntegerValue(flag)
	...
	private[this] def fromValue(v:Value):Any = v match {
		...
		case value if value.isMapValue =>
			...
			(for(i <- 0 until kv.length by 2) yield {
				(fromValue(kv(i)), fromVaule(kv(i+1)))
			}).toMap
	}
